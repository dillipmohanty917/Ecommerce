...
$(document).ready(() => {
  ...
  let $discountTypeInput = $('.body-discounts [name="type"]');
  if ($discountTypeInput.length) {
    let $discountValueType = $('[name="discount_value_type"]');
    let $discountForms = $('.discount-form');
    let $applyToProduct = $('[name="product-apply_to"]').parents('.input');
    let $applyToCategory = $('[name="category-apply_to"]').parents('.input');
    let onChange = () => {
      let discountValueType = $discountValueType.val();
      let type = $discountTypeInput.val();
      let hide = discountValueType === 'percentage';
      $applyToProduct.toggleClass('hide', hide);
      $applyToCategory.toggleClass('hide', hide);

      $discountForms.each((index, form) => {
        let $form = $(form);
        let hideForm = $form.data('type') !== type;
        $form.toggleClass('hide', hideForm);
      });
    ...
    $discountValueType.on('change', onChange);
    $discountTypeInput.on('change', onChange);
    $discountTypeInput.trigger('change');
  }

  let $schemeTypeInput = $('.body-schemes [name="type"]');
  if ($schemeTypeInput.length) {
    let $schemeValueType = $('[name="scheme_value_type"]');
    let $schemeForms = $('.scheme-form');
    let $applyToProduct = $('[name="product-apply_to"]').parents('.input');
    let $applyToCategory = $('[name="category-apply_to"]').parents('.input');
    let onChange = () => {
      let schemeValueType = $schemeValueType.val();
      let type = $schemeTypeInput.val();
      let hide = schemeValueType === 'percentage';
      $applyToProduct.toggleClass('hide', hide);
      $applyToCategory.toggleClass('hide', hide);

      $schemeForms.each((index, form) => {
        let $form = $(form);
        let hideForm = $form.data('type') !== type;
        $form.toggleClass('hide', hideForm);
      });
    ...
    $schemeValueType.on('change', onChange);
    $schemeTypeInput.on('change', onChange);
    $schemeTypeInput.trigger('change');
  }


  /*deposit form jquery*/
  let $depositTypeInput = $('.body-logistics [name="mode"]');
  if ($depositTypeInput.length) {
    let $depositForms = $('.deposit-form');
    let $applyToBank = $('[name="mode-Bank"]').parents('.input');
    let $applyToUser = $('[name="mode-User"]').parents('.input');
    let onChange = () => {
      let type = $depositTypeInput.val();
      $applyToBank.toggleClass('hide', true);
      $applyToUser.toggleClass('hide', true);
      $depositForms.each((index, form) => {
        let $form = $(form);
        let hideForm = $form.data('type') !== type;
        $form.toggleClass('hide', hideForm);
      });
    };
    $depositTypeInput.on('change', onChange);
    $depositTypeInput.trigger('change');
  }
});
